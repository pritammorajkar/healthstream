...
interface StatCardProps {
  ...
  subtext: string
  subtextIcon?: string
  trend?: number        // positive = green arrow, undefined = no trend
  icon: LucideIcon
  loading?: boolean
}
  ...
  return (
    <div className="rounded-xl bg-white border border-gray-100 shadow-sm p-6 flex flex-col gap-2">
      <div className="flex items-start justify-between">
        ...
        <Icon className="h-5 w-5 text-[#1B3F8B]" strokeWidth={1.75} />
      </div>
      ...
      <div className="mt-2 h-1 w-full rounded-full bg-blue-100">
        <div className="h-1 rounded-full bg-[#1B3F8B]" style={{ width: '70%' }} />
      </div>
    </div>
  )
}
